...
- **Storage Layer**: 계층적 폴더 구조와 XML 파일 기반의 영속성 계층.
- **Version Control Layer**: 문서 수정 이력 관리 및 스냅샷 복구.
- **Editor Layer**: Tiptap 기반의 WYSIWYG 편집 및 스타일 엔진.
- **Collaboration & AI Layer**: 실시간 동기화 및 LLM 기반 지능형 도구.
...
- **Folder/File Hierarchy**: 파일 시스템은 가상 디렉토리 구조를 지원합니다. 모든 폴더와 파일 정보는 메타데이터 DB(예: PostgreSQL)에서 관리되며, 실제 본문은 파일 저장소에 XML 텍스트로 저장됩니다.
- **Permission Control**: 각 문서/폴더별로 '소유자(Owner)', '편집자(Editor)', '조회자(Viewer)' 권한을 설정할 수 있습니다.

### 2.2 문서 버전 관리 (Version Control System)

- **Snapshotting**: 주요 변경 시점마다 문서의 XML 전체 상태를 스냅샷으로 저장합니다.
- **Time-travel Editing**: Yjs의 `Y.UndoManager`를 통해 실시간 편집 내역(Undo/Redo)을 추적하고, 특정 시점의 문서 상태로 롤백할 수 있는 기능을 제공합니다.
- **Binary Text Diff**: 텍스트 기반 XML 형식을 활용하여 버전 간 차이점(Diff)을 시각적으로 표시합니다.
...
- **Full-text Search**: 저장된 XML 텍스트 내에서 키워드를 검색합니다. (Elasticsearch 또는 인덱싱 엔진 연동)
- **Semantic Search**: AI 벡터 임베딩을 활용하여 단어의 의미적 유사성을 바탕으로 문서를 검색하는 기능을 지원합니다.

### 2.5 AI 지능형 편집 (AI-Powered Editing)

- **Context-aware Assistance**: 현재 편집 중인 섹션의 XML 구조와 상위 문맥을 LLM에 전달하여 가장 적절한 이어쓰기나 수정을 제안합니다.
...
1. **사용자 접속**: 로그인 후 폴더 트리를 통해 파일 접근 권한 확인.
2. **문서 로드**: 서버에서 XML 데이터 로드 -> JSON 변환 -> 에디터 상태 주입.
3. **편집 중**: 실시간 Yjs 동기화 + 주기적인 자동 저장(XML 변환 후 파일 저장).
