...
export async function GET(request) {
  ...
  if (!documentId || !timestamp || !format) {
    return new Response(
      JSON.stringify({
        error: "Document ID, timestamp, and format are required.",
      }),
      {
        status: 400,
  ...
  try {
    if (format === "binary") {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Invalid format specified. Must be binary, delta, or markdown.",
        }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
    ...
    console.error(
      `Error retrieving version content for document ${documentId}, timestamp ${timestamp}, format ${format}:`,
      error,
    );
    return new Response(
      JSON.stringify({ error: "Failed to retrieve version content." }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      },
